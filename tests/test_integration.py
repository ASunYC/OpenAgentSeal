...
from open_agent.tools.note_tool import RecallNoteTool, SessionNoteTool
...
async def test_basic_agent_usage():
    ...
    # Load configuration
    config_path = Path("open_agent/config/config.yaml")
    ...
    with tempfile.TemporaryDirectory() as workspace_dir:
        ...
        try:
            ...
            mcp_tools = await load_mcp_tools_async(
                config_path="open_agent/config/mcp.json"
            )
            if mcp_tools:
        ...
        task = """
        Create a Python file named hello.py in the workspace that prints "Hello, Open Agent!".
