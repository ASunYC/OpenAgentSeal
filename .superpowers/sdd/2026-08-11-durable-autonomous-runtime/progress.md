...
Task 14: RED 9e3f908 (official gateway connector contracts and failing conformance collection)
Task 14: fix round 1/5 (3 HIGH and 2 MEDIUM code findings addressed: same-reference credential restart, lease-loss session termination, bounded Discord resume hosts, INVALID_SESSION resumability, and prompt shutdown cancellation)
Task 14: fix round 2/5 (1 HIGH and 2 MEDIUM code findings addressed: cancellation-safe child reaping, strict regional resume hosts, and official INVALID_SESSION delay; code re-review clean)
Task 14: security fix round 1/5 (2 HIGH and 1 MEDIUM addressed: durable-only resume sequence publication, credential-publication session invalidation, and bounded session replay history; security re-review clean)
Task 14: fix round 3/5 (post-publication backend failure now invalidates the pre-existing connector session; final code delta review clean)
Task 14: verification (31 connector tests at 83% connector coverage; reviewer-focused 31 connector + 40 supervisor + 3 credential API tests; 336 gateway/runtime/API compatibility tests and 8 autonomous E2E tests passed; final code/security 0 Critical / 0 High / 0 Medium)
