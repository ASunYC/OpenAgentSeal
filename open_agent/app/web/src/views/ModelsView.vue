<template>
  <div class="models-view">
    <header class="view-header">
      <h1>{{ t('大模型配置', 'Model Configuration') }}</h1>
      <p class="subtitle">{{ t('配置不同提供商的大模型API', 'Configure LLM APIs from different providers') }}</p>
    </header>
    ...
    <div class="models-grid">
      ...
      >
        <div class="card-header">
            ...
            <div class="provider-name">
              ...
              <template v-if="config.isNew">
                <select 
                  ...
                  @change="onProviderChange(config.id, ($event.target as HTMLSelectElement).value)"
                >
              ...
              <template v-else>
                <h3>{{ config.provider_display_name || config.provider }}</h3>
          ...
          <div class="card-actions">
            ...
            <button
              ...
              @click="config.isNew ? removeConfig(config.id) : deleteConfig(config.id)"
              :title="t('删除', 'Delete')"
            >
        ...
        <div class="card-content">
          <div class="form-group">
            ...
            <div class="input-with-toggle">
              <input 
                ...
                :value="config.apiKey"
                @input="updateApiKey(config.id, ($event.target as HTMLInputElement).value)"
          ...
          <div class="form-group" v-if="config.isNew || config.editing">
            ...
            <input 
              ...
              @input="updateBaseUrl(config.id, ($event.target as HTMLInputElement).value)"
          ...
          <div class="form-group">
            ...
            <input 
              ...
              @input="updateSelectedModel(config.id, ($event.target as HTMLInputElement).value)"
          ...
          <div class="form-group">
            ...
            <div class="model-tags">
              <span 
                ...
                @click="selectModel(config.id, model)"
              >
                {{ model }}
              </span>
              <!-- 刷新模型列表按钮 -->
          ...
          <div class="form-group" v-if="config.editing || config.isNew">
            ...
            <div class="input-with-btn">
              <input 
                ...
                @input="updateNewModel(config.id, ($event.target as HTMLInputElement).value)"
                :placeholder="t('输入模型名称', 'Enter model name')"
                @keyup.enter="addModel(config.id)"
              />
              <button class="btn-add" @click="addModel(config.id)">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
        ...
        <div class="card-footer">
          <button class="btn-save" @click="saveConfig(config.id)" :disabled="config.saving || (config.isNew && !config.provider)">
            {{ config.saving ? t('保存中...', 'Saving...') : t('保存配置', 'Save Config') }}
          </button>
        </div>
      </div>
...
function removeConfig(id: string) {
  ...
  if (index !== -1 && modelConfigs[index].isNew) {
...
async function deleteConfig(id: string) {
  ...
  try {
    const result = await api.deleteModelConfig(id)
    if (result.success) {
      // 从本地列表中移除
      const index = modelConfigs.findIndex(c => c.id === id)
      if (index !== -1) {
...
// 提供商变更时更新相关信息
async function onProviderChange(id: string, provider: string) {
  const config = modelConfigs.find(c => c.id === id)
  if (!config) return
...
function updateBaseUrl(id: string, url: string) {
  const config = modelConfigs.find(c => c.id === id)
  if (config) {
    config.base_url = url
  }
...
function updateSelectedModel(id: string, model: string) {
  const config = modelConfigs.find(c => c.id === id)
  if (config) {
...
function updateNewModel(id: string, model: string) {
  const config = modelConfigs.find(c => c.id === id)
  if (config) {
    config.newModel = model
  }
}
...
async function addModel(id: string) {
  const config = modelConfigs.find(c => c.id === id)
  if (config && config.newModel && config.newModel.trim()) {
    if (!config.models.includes(config.newModel.trim())) {
      config.models.push(config.newModel.trim())
    }
    // 自动选中新添加的模型
    config.selectedModel = config.newModel.trim()
    config.newModel = ''
  }
}
...
async function saveConfig(id: string) {
  ...
  try {
    ...
    const modelConfig: ModelConfig = {
      ...
      name: config.selectedModel || config.name,
    ...
    if (result.success) {
      ...
      await agentStore.loadModelConfigs()
      alert(t('配置已保存', 'Configuration saved'))
    } else {
