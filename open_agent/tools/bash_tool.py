...
Examples:
  ...
  - python3 -m http.server 8080 (with run_in_background=true)""",
    ...
    def _get_venv_env(self) -> dict[str, str] | None:
        """Get environment variables for virtual environment if configured.
        
        Returns:
            Modified environment dict with venv paths prepended, or None if no venv.
        """
    ...
    async def execute(
        ...
        try:
            ...
            # Get venv environment if configured
            venv_env = self._get_venv_env()

            # Prepare shell-specific command execution
            if self.is_windows:
                # Windows: Use PowerShell with UTF-8 encoding to prevent Chinese garbled text
                # Set OutputEncoding and [Console]::OutputEncoding to UTF-8 for proper Chinese display
                utf8_command = f"[Console]::OutputEncoding = [System.Text.Encoding]::UTF8; $OutputEncoding = [System.Text.Encoding]::UTF8; {command}"
                shell_cmd = ["powershell.exe", "-NoProfile", "-Command", utf8_command]
            else:
