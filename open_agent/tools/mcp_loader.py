...
class MCPTool(Tool):
    ...
    def description(self) -> str:
        # Add [MCP] prefix to make MCP tools easily identifiable
        server_info = f" ({self._server_name})" if self._server_name else ""
        return f"[MCP{server_info}] {self._description}"
...
class MCPPromptInfo:
    ...
    name: str
    description: str
    arguments: list[dict[str, Any]]  # List of argument definitions
    server_name: str
...
class MCPServerConnection:
    ...
    def __init__(
        ...
        # MCP Prompts (usage guides provided by the server)
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            # Load available prompts (usage guides from MCP server)
            try:
                ...
                for prompt in prompts_list.prompts:
                    prompt_info = MCPPromptInfo(
                        ...
                        arguments=[
                            {"name": arg.name, "description": arg.description or "", "required": arg.required if hasattr(arg, "required") else False}
    ...
    async def disconnect(self):
        ...
        if self.exit_stack:
            try:
                ...
                # task context during shutdown. This is a known issue in the
                # MCP Python SDK and anyio library.
                pass
            finally:
                self.exit_stack = None
                self.session = None
...
def get_all_mcp_prompts() -> list[MCPPromptInfo]:
    """Get all available MCP prompts from all connected servers."""
    all_prompts = []
    for connection in _mcp_connections:
        all_prompts.extend(connection.prompts)
    return all_prompts


async def get_mcp_prompt(prompt_name: str, arguments: dict[str, str] | None = None) -> str:
    """
    Get the content of an MCP prompt (usage guide).
        ...
        for prompt in connection.prompts:
            if prompt.name == prompt_name:
                if connection.session is None:
                    return f"Error: Connection to '{connection.name}' is not active"
                
                try:
                    result = await connection.session.get_prompt(prompt_name, arguments=arguments)
                    # Build prompt content from messages
                    content_parts = []
                    ...
                    return "\n\n".join(content_parts) if content_parts else "Empty prompt"
                except Exception as e:
                    return f"Error retrieving prompt '{prompt_name}': {e}"
    
    return f"Prompt '{prompt_name}' not found in any connected MCP server"


class ListMCPPromptsTool(Tool):
    """Tool to list all available MCP prompts (usage guides)."""
    ...
    @property
    def description(self) -> str:
        return "List all available MCP prompts (usage guides) from connected MCP servers. Use this to discover what guides are available for MCP tools."
    ...
    async def execute(self) -> ToolResult:
        ...
        if not prompts:
            return ToolResult(
                success=True,
                content="No MCP prompts available. MCP servers may not provide usage guides, or no MCP servers are connected."
            )
        
        lines = ["## Available MCP Prompts (Usage Guides)\n"]
        for prompt in prompts:
            lines.append(f"### {prompt.name} (from {prompt.server_name})")
        ...
        lines.append("Use `get_mcp_prompt` with the prompt name to retrieve the full guide content.")
        
        return ToolResult(success=True, content="\n".join(lines))


class GetMCPPromptTool(Tool):
    """Tool to get the content of a specific MCP prompt."""
    ...
    @property
    def description(self) -> str:
        return "Get the content of an MCP prompt (usage guide). Use this to learn how to use MCP tools properly. First use list_mcp_prompts to see available prompts."
    ...
    def parameters(self) -> dict[str, Any]:
        return {
            ...
            "properties": {
                "prompt_name": {
                    "type": "string",
                    "description": "Name of the prompt to retrieve (use list_mcp_prompts to see available names)"
                },
                "arguments": {
                    "type": "object",
    ...
    async def execute(self, prompt_name: str, arguments: dict[str, str] | None = None) -> ToolResult:
        content = await get_mcp_prompt(prompt_name, arguments)
        success = not content.startswith("Error:")
        return ToolResult(success=success, content=content, error=None if success else content)
