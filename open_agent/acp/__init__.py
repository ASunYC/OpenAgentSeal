"""ACP (Agent Client Protocol) bridge for open-agent."""
...
from open_agent.cli import add_workspace_tools, initialize_base_tools
...
class MiniMaxACPAgent:
    ...
    async def initialize(self, params: InitializeRequest) -> InitializeResponse:  # noqa: ARG002
        return InitializeResponse(
            ...
            agentInfo=Implementation(name="open-agent", title="open-agent", version="0.1.0"),
        )
...
async def run_acp_server(config: Config | None = None) -> None:
    """Run open-agent as an ACP-compatible stdio server."""
    ...
    logger.info("open-agent ACP server running")
