...
from .utils.path_utils import get_external_config_dir, get_executable_dir, is_frozen
...
def get_user_app_dir() -> Path:
    ...
    This should match the logic in launcher.py and path_utils.py
...
class LLMConfig(BaseModel):
    ...
    Note: api_key, api_base, model, provider are now loaded from open_agent.json
    via CLI model selection. These fields are kept for backward compatibility
    but default to empty/placeholder values.
    """

    api_key: str = ""  # Loaded from open_agent.json at runtime
    api_base: str = ""
    model: str = ""
...
class Config(BaseModel):
    ...
    def load(cls) -> "Config":
        ...
        if not config_path.exists():
            raise FileNotFoundError("Configuration file not found. Run scripts/setup-config.sh or place config.yaml in open_agent/config/.")
    ...
    def from_yaml(cls, config_path: str | Path) -> "Config":
        ...
        # Note: api_key, api_base, model, provider are now managed via open_agent.json
        # Use default LLMConfig with retry settings
        llm_config = LLMConfig(
            api_key=data.get("api_key", ""),  # Empty by default, loaded from open_agent.json
    ...
    def get_package_dir() -> Path:
        ...
        Returns:
            Path to the open_agent package directory
        """
    ...
    def find_config_file(cls, filename: str) -> Path | None:
        ...
        1) config/{filename} next to executable (external config directory)
        2) User app directory config/{filename} (~/.open-agent/config/)

        When running in development mode:
        1) open_agent/config/{filename} in current directory (development mode)
        2) User app directory config/{filename} (~/.open-agent/config/)
        3) {package}/open_agent/config/{filename} in package installation directory
        ...
        if is_frozen():
            ...
            external_config = get_external_config_dir()
            if external_config:
                config_file = external_config / filename
                if config_file.exists():
                    return config_file
            
            # Priority 2: User app directory config/
        ...
        # Development mode - use original priority order
        # Priority 1: Development mode - current directory's config/ subdirectory
        dev_config = Path.cwd() / "open_agent" / "config" / filename
